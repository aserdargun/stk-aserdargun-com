...
test("serializes a validated prebuilt production artifact into the stk Static Web App", () => {
  ...
  assert.match(azureWorkflow, /npm run build:api:artifact/);
  assert.match(azureWorkflow, /test -s api-dist\/host\.json/);
  assert.match(azureWorkflow, /test -s api-dist\/package\.json/);
  assert.match(azureWorkflow, /test -d api-dist\/node_modules\/\@azure\/functions/);
  assert.match(azureWorkflow, /app_location: dist/);
  assert.match(azureWorkflow, /api_location: api-dist/);
  assert.match(azureWorkflow, /skip_app_build: true/);
  assert.match(azureWorkflow, /skip_api_build: true/);
